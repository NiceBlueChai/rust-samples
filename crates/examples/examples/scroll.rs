...
use fake::Fake;
use gpui::{
    App, Application, Bounds, Context, ElementId, Entity, FocusHandle, Focusable, Pixels,
    SharedString, Size, Window, WindowBounds, WindowOptions, actions, div, px, size,
};
use gpui::{
    AppContext, InteractiveElement, IntoElement, ParentElement, Render, RenderOnce, ScrollStrategy,
    Styled, Subscription, Task, Timer,
};

use gpui_component::{
    ActiveTheme, Icon, IconName, IndexPath, Root, Selectable, Sizable,
    button::Button,
    checkbox::Checkbox,
    h_flex,
    label::Label,
...
pub fn create_new_window_with_size<F, V>(
    _title: &str,
...
fn main() {
    Application::new().run(|cx: &mut App| {
        ...
        create_new_window_with_size(
            "Scroll Example",
            None,
            |window, cx| ListStory::view(window, cx),
            cx,
        );
        cx.activate(true);
    });
